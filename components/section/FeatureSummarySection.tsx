...
function FeatureCard(props: FeatureCardProps) {
    ...
    return (
        <Card.Root w={{ base: 52, md: 52, lg: 72 }} variant="elevated" key={title}>
            <Card.Body textAlign={"center"} px={{ base: 6, md: 6, lg: 8 }} py={{ base: 3, md: 4, lg: 6 }} gap={{ base: 2, md: 2 }} justifyContent={"flex-start"}>
                <Flex justify={'center'}>
                    <Box w={{ base: 12, md: 16 }} h={{ base: 12, md: 16 }} position={'relative'}>
                        <Box zIndex={10} position='absolute'><props.renderIcon size={48} strokeWidth={1} /></Box>
                ...
                <Card.Title mb="2" as="div"><Heading as="h3" fontSize={{ base: 24, lg: 28 }} >{title}</Heading></Card.Title>
    ...
    return (
        <Section pt={{ base: 10, md: 10 }}>
            <Stack gap={{ base: 4, md: 4 }} maxWidth={"7xl"} width={"100%"}>
                ...
                <Stack direction={{ base: 'row', lg: 'row' }} gap={{ base: 'auto', md: 'auto' }} py={{ base: 2, md: 2 }} justifyContent={{ base: "space-between", md: "space-between" }}>
